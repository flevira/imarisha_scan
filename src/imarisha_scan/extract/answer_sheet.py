...
class AnswerSheetExtractor:
    ...
    def extract_rows(self, ocr_text: str, qr_payload: str, answers_by_question: dict[str, str]) -> list[dict[str, str]]:
        context = self.parse_qr_payload(qr_payload)

        user_id = self._extract_user_id(ocr_text) or context.user_id
        question_ids = self._extract_question_ids(ocr_text)
        rows: list[dict[str, str]] = []

        for question_id in question_ids:
            answer = answers_by_question.get(question_id, "").strip().upper()
            row = {
                "user_id": user_id,
